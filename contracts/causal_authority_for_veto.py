...
def _pearl_veto_authority_bundle(contract: Optional[Dict[str, str]]) -> Dict[str, Any]:
    """Return the Step-78 Pearl-complete evidence bundle for a veto card.

    This is intentionally stricter than causal-veto authority.  Existing
    contracts can authorize path-specific causal language, but Pearl-complete
    language remains blocked unless the offline backend explicitly exposes full
    recursive-ID completion, a passed Full-ID readiness matrix, oracle/fuzz
    parity, and complete failure-certificate coverage.
    """
    row = dict(contract or {})
    coverage_raw = row.get("formal_failure_certificate_coverage_complete")
    if coverage_raw is None:
        coverage_raw = row.get("formal_failure_certificate_coverage", "")
    coverage_complete = _truthy_contract_flag(coverage_raw)
    primary_formula_authority = row.get("primary_formula_authority", row.get("authority_level", ""))
    formula_authority_allowed = primary_formula_authority in PEARL_COMPLETE_FORMULA_AUTHORITIES
    bundle = {
        "full_recursive_id_implemented": int(_truthy_contract_flag(row.get("full_recursive_id_implemented", 0))),
        "full_id_claim_allowed": int(_truthy_contract_flag(row.get("full_id_claim_allowed", 0))),
        "id_full_readiness_matrix": row.get("id_full_readiness_matrix", ""),
        "id_full_readiness_matrix_passed": int(_truthy_contract_flag(row.get("id_full_readiness_matrix_passed", 0))),
        "id_oracle_parity_matrix": row.get("id_oracle_parity_matrix", ""),
        "id_oracle_parity_fuzz_passed": int(_truthy_contract_flag(row.get("id_oracle_parity_fuzz_passed", 0))),
        "formal_failure_certificate_coverage_complete": int(coverage_complete),
        "id_full_promotion_gate": row.get("id_full_promotion_gate", ""),
        "id_full_promotion_gate_passed": int(_truthy_contract_flag(row.get("id_full_promotion_gate_passed", 0))),
        "id_global_full_id_controlled_flip": row.get("id_global_full_id_controlled_flip", ""),
        "id_global_full_id_controlled_flip_passed": int(_truthy_contract_flag(row.get("id_global_full_id_controlled_flip_passed", row.get("global_full_id_controlled_flip_allowed", 0)))),
        "global_full_id_controlled_flip_allowed": int(_truthy_contract_flag(row.get("global_full_id_controlled_flip_allowed", row.get("id_global_full_id_controlled_flip_passed", 0)))),
        "formal_failure_certificate_coverage": "complete" if coverage_complete else str(row.get("formal_failure_certificate_coverage", "not_asserted") or "not_asserted"),
        ...
        "pearl_claim_source": "causal_contract_full_id_readiness_fields" if row else "no_matching_contract",
    }
    required = [
    ...
    bundle["missing_required_flags"] = [k for k in required if not _truthy_contract_flag(bundle.get(k))]
    bundle["pearl_complete_evidence_bundle_present"] = int(not bundle["missing_required_flags"])
    return bundle


def _scoped_pearl_manifest_from_contract_row(row: Dict[str, str], bundle: Dict[str, Any]) -> Dict[str, Any]:
    """Build a Step-88 scope manifest for an offline card when fields are green.

    The card builder is offline, so it may bind scoped Pearl authority to the
    current audited-surface manifest.  If the contract row does not carry the
    needed scoped-Pearl evidence, return an empty manifest and runtime will block
    the scoped Pearl wording.
    """
    if not row:
        return {}
    if not _truthy_contract_flag(bundle.get("scoped_full_recursive_id_implemented")):
        return {}
    if not _truthy_contract_flag(bundle.get("scoped_full_id_claim_allowed")):
        return {}
    if not _truthy_contract_flag(bundle.get("id_scoped_pearl_promotion_gate_passed")):
        return {}
    if not _truthy_contract_flag(bundle.get("pearl_scoped_formula_authority_allowed")):
        return {}
    gate = {
        "matrix_version": bundle.get("id_scoped_pearl_promotion_gate", ""),
        "scope": bundle.get("scope", ""),
        "scoped_promotion_allowed": bundle.get("id_scoped_pearl_promotion_gate_passed", 0),
        "scoped_pearl_veto_claim_allowed": bundle.get("id_scoped_pearl_promotion_gate_passed", 0),
        "id_full_readiness_matrix": bundle.get("id_full_readiness_matrix", ""),
        "id_full_readiness_matrix_passed": bundle.get("id_full_readiness_matrix_passed", 0),
        "id_oracle_parity_matrix": bundle.get("id_oracle_parity_matrix", ""),
        "id_oracle_parity_fuzz_passed": bundle.get("id_oracle_parity_fuzz_passed", 0),
        "formal_failure_certificate_coverage_complete": bundle.get("formal_failure_certificate_coverage_complete", 0),
        "failure_certificate_coverage_matrix": row.get("failure_certificate_coverage_matrix", row.get("id_failure_certificate_coverage_matrix", "")),
        "full_recursive_formula_authority_coverage_matrix": row.get("full_recursive_formula_authority_coverage_matrix", row.get("id_full_recursive_authority_coverage_matrix", "")),
        "pearl_formula_authority_coverage_complete": row.get("pearl_formula_authority_coverage_complete", 0),
        "n_identified_formula_rows_audited": row.get("n_identified_formula_rows_audited", 0),
        "n_full_recursive_formula_authority": row.get("n_full_recursive_formula_authority", 0),
        "n_finite_or_template_formula_authority": row.get("n_finite_or_template_formula_authority", 0),
        "n_technical_pending_not_certified": row.get("n_technical_pending_not_certified", 0),
        "global_full_recursive_id_implemented": bundle.get("global_full_recursive_id_implemented", 0),
        "global_full_id_claim_allowed": bundle.get("global_full_id_claim_allowed", 0),
    }
    manifest = build_scoped_pearl_manifest_from_gate(gate)
    if manifest.get("manifest_version") != PEARL_SCOPED_SCOPE_MANIFEST_VERSION:
        return {}
    return manifest


def _pearl_scoped_veto_authority_bundle(contract: Optional[Dict[str, str]]) -> Dict[str, Any]:
    """Return the Step-87 scoped Pearl evidence bundle for a veto card.

    This bundle is narrower than Pearl-complete authority.  It allows cards to
    carry a scope-bound claim for the audited public surface while preserving the
    global Full-ID flags at whatever value the backend contract exposes.
    """
    row = dict(contract or {})
    coverage_raw = row.get("formal_failure_certificate_coverage_complete")
    if coverage_raw is None:
    ...
    coverage_complete = _truthy_contract_flag(coverage_raw)
    primary_formula_authority = row.get("primary_formula_authority", row.get("authority_level", ""))
    formula_authority_allowed = primary_formula_authority in PEARL_SCOPED_FORMULA_AUTHORITIES
    bundle = {
        "scoped_full_recursive_id_implemented": int(_truthy_contract_flag(row.get("scoped_full_recursive_id_implemented", row.get("audited_pearl_id_implemented", 0)))),
        "scoped_full_id_claim_allowed": int(_truthy_contract_flag(row.get("scoped_full_id_claim_allowed", row.get("audited_pearl_claim_allowed", 0)))),
        "global_full_recursive_id_implemented": int(_truthy_contract_flag(row.get("global_full_recursive_id_implemented", row.get("full_recursive_id_implemented", 0)))),
        "global_full_id_claim_allowed": int(_truthy_contract_flag(row.get("global_full_id_claim_allowed", row.get("full_id_claim_allowed", 0)))),
        "id_full_readiness_matrix": row.get("id_full_readiness_matrix", ""),
        "id_full_readiness_matrix_passed": int(_truthy_contract_flag(row.get("id_full_readiness_matrix_passed", 0))),
        "id_oracle_parity_matrix": row.get("id_oracle_parity_matrix", ""),
        "id_oracle_parity_fuzz_passed": int(_truthy_contract_flag(row.get("id_oracle_parity_fuzz_passed", 0))),
        "formal_failure_certificate_coverage_complete": int(coverage_complete),
        "formal_failure_certificate_coverage": "complete" if coverage_complete else str(row.get("formal_failure_certificate_coverage", "not_asserted") or "not_asserted"),
        "id_scoped_pearl_promotion_gate": row.get("id_scoped_pearl_promotion_gate", ""),
        "id_scoped_pearl_promotion_gate_passed": int(_truthy_contract_flag(row.get("id_scoped_pearl_promotion_gate_passed", 0))),
        "scope": row.get("scope", row.get("pearl_claim_scope", "")),
        "primary_formula_authority": primary_formula_authority,
        ...
        "identification_status": row.get("identification_status", ""),
        "pearl_claim_source": "causal_contract_scoped_pearl_readiness_fields" if row else "no_matching_contract",
    }
    bundle["scoped_pearl_manifest"] = _scoped_pearl_manifest_from_contract_row(row, bundle)
    bundle["scoped_pearl_manifest_version"] = str((bundle.get("scoped_pearl_manifest") or {}).get("manifest_version", ""))
    bundle["scoped_pearl_manifest_digest"] = str((bundle.get("scoped_pearl_manifest") or {}).get("manifest_digest", ""))
    bundle["scoped_pearl_manifest_verified"] = int(bool(bundle["scoped_pearl_manifest_digest"]))
    bundle["scoped_pearl_release_manifest"] = (
        build_scoped_pearl_release_manifest(bundle["scoped_pearl_manifest"])
        if bundle["scoped_pearl_manifest_verified"]
        else {}
    )
    bundle["scoped_pearl_release_manifest_version"] = str((bundle.get("scoped_pearl_release_manifest") or {}).get("release_manifest_version", ""))
    bundle["scoped_pearl_release_manifest_digest"] = str((bundle.get("scoped_pearl_release_manifest") or {}).get("release_manifest_digest", ""))
    bundle["scoped_pearl_release_verified"] = int(bool(bundle["scoped_pearl_release_manifest_digest"]))
    required = [
    ...
    bundle["missing_required_flags"] = [k for k in required if not _truthy_contract_flag(bundle.get(k))]
    bundle["pearl_scoped_evidence_bundle_present"] = int(not bundle["missing_required_flags"])
    return bundle
...
def build_causal_authority_cards(
    ...
    for outcome_key, hint_obj in sorted(path_hints.items(), key=lambda kv: str(kv[0])):
        ...
        card = {
            ...
            "pearl_veto_authority": _pearl_veto_authority_bundle(contract),
            "pearl_scoped_veto_authority": _pearl_scoped_veto_authority_bundle(contract),
