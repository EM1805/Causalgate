...
def _instruction_for(decision: str, action: ActionPackage, reason: str) -> str:
    ...
    if decision == "warn":
        return f"Proceed carefully with {name}; include a short warning or mitigation because CausalGate detected elevated risk."
    if decision == "ask_clarification":
        return f"Do not execute {name} yet; ask the user for confirmation or missing information first. Reason: {reason}"
    if decision == "veto":
        return f"Do not execute {name}; CausalGate vetoed it. Explain the safety reason and offer a safer alternative."
    return f"Abstain from {name}; causal/safety support is insufficient. Ask for more evidence or route to review."


@dataclass
class DecisionPackage:
    """Canonical agent-facing output of CausalGate's Decision Gate."""
