...
"""Stable online SCM-ID adapter for CausalGate.

This facade accepts compact agent/runtime graph payloads and routes them into
CausalGate's existing SCM-ID / Full-ID-shaped backend without letting backend
...
def _global_full_id_claim_allowed() -> int:
    flags = id_capability_flags()
    return int(flags.get("full_recursive_id_implemented") == 1 and flags.get("full_id_claim_allowed") == 1)
...
class IdentificationQuery:
    ...
    strategy_hint: str = ""
    query_id: str = ""
    source: str = "causalgate.causal_core.identification"
    max_depth: int = 8
    ...
    def from_payload(cls, payload: Mapping[str, Any] | None) -> "IdentificationQuery":
        ...
        return cls(
            ...
            source=_s(payload.get("source")) or "causalgate.causal_core.identification",
...
def _blocked_result(
    *,
    ...
    return IdentificationResult(
        ...
        full_id_claim_allowed=_global_full_id_claim_allowed(),
        reason=reason,
...
def _map_graphical(query: IdentificationQuery, diagnostic: Any) -> IdentificationResult:
    ...
    return IdentificationResult(
        ...
        full_id_claim_allowed=int(raw.get("full_id_claim_allowed") or _global_full_id_claim_allowed()),
        reason=_s(raw.get("reason_codes")) or strategy,
    ...
    if invalid_reasons or open_backdoor_paths:
        ...
        return _blocked_result(
            ...
            reason=(
                ...
                "CausalGate will not promote an LLM-supplied adjustment set to a causal identification claim."
            ),
