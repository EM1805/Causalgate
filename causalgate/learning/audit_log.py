...
class AuditLog:
    """Small stdlib-only JSONL logger for online CausalGate decisions."""
    ...
    def append_event(self, event: AuditEvent | Mapping[str, Any]) -> Dict[str, Any]:
        ...
        record.setdefault("source", "causalgate_online")
...
def build_decision_event(run_result: Mapping[str, Any]) -> AuditEvent:
    ...
    return AuditEvent(
        ...
        source=_clean_str(input_package.get("source") or llm_response.get("source") or "causalgate_online"),
        payload={
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Append/read CausalGate Learning Loop audit events.")
