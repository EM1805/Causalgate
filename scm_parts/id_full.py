...
"""Full-ID public scaffold and canonical control-flow overlay for CausalGate.

Step 80 keeps the canonical control-flow shell, owned canonical formula authority, operational carried-Q AST, conservative IDC, canonical ID-4/ID-7 special-family formulas, standardized failure certificates, the first non-frontdoor arbitrary carried-Q marginalization branch, a strict downstream-confounded chain ID-7 formula authority family, and a finite audited fuzz-closure authority for six previously delegated recursive ID-7 formulas. It still does **not** claim arbitrary Full ID/IDC: formula authority remains gated until broad oracle parity, full hedge coverage, and a broad IDC matrix are complete.
...
from .id_step80_audited_fuzz import ID_STEP80_AUDITED_FUZZ_AUTHORITY, is_step80_audited_fuzz_closure
...
ID_FULL_INTERFACE_VERSION = "id_full_interface_v4_step80"
ID_FULL_IMPLEMENTATION_LEVEL = (
    "canonical_id_1_to_7_control_flow_overlay_with_owned_formula_authority_for_ID_1_ID_2_ID_4_ID_6_gated_ID_7_frontdoor_contextual_chain_parallel_frontdoor_failure_certificate_authority_step68_idc_pruning_step62_do_proof_metadata_step73_canonical_templates_step74_arbitrary_id7_carried_q_marginalization_step75_downstream_confounded_chain_formula_step79_audited_fuzz_closure_step80_no_full_id_claim"
)
...
def _expression_uses_step75_carried_q_marginalization(expr: RecursiveIDExpressionDiagnostic) -> int:
    return int(
        "q_input_ancestor_marginalization_step75" in (expr.trace_json or "")
        or "Q_INPUT_OBSERVED_DAG_TRUNCATED_FACTORIZATION_STEP75" in (expr.reason_codes or "")
        or "arbitrary_id7_carried_q_recursion_step75" in (expr.expression_json or "")
    )
...
def full_id(
    ...
    *,
    ...
    """Run CausalGate's Full-ID-shaped set-valued identification entrypoint.
    ...
    claim_allowed = int(caps.get("full_recursive_id_implemented") == 1 and caps.get("full_id_claim_allowed") == 1)
    ...
    claim_reason = (
        ...
        if claim_allowed
        else "general_ID_and_IDC_not_complete_yet;canonical_ID_1_ID_2_ID_4_ID_6_gated_ID7_frontdoor_contextual_step75_carried_Q_marginalization_step79_downstream_confounded_chain_step80_audited_fuzz_closure_and_failure_certificates_ready"
    )
    step75_formula_authority = bool(delegate_identified and not use_canonical_formula and _expression_uses_step75_carried_q_marginalization(expr))
    step80_audited_fuzz_authority = bool(delegate_identified and not use_canonical_formula and is_step80_audited_fuzz_closure(admg, x, y))
    primary_authority = (
        ID_CANONICAL_FORMULA_AUTHORITY
        if use_canonical_formula
        else (
            ID_FAILURE_CERTIFICATE_AUTHORITY
            if failure
            else (
                "recursive_id_set_expression_diagnostic_step75"
                if step75_formula_authority
                else (ID_STEP80_AUDITED_FUZZ_AUTHORITY if step80_audited_fuzz_authority else "recursive_id_set_expression_diagnostic")
            )
        )
    )
    ...
    return FullIDDiagnostic(
        ...
        primary_formula_authority=primary_authority,
