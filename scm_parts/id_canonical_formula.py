...
ID_CANONICAL_FORMULA_VERSION = "id_canonical_formula_v7_step79"
ID_CANONICAL_FORMULA_AUTHORITY = "id_canonical_formula_step60"
ID_CANONICAL_FORMULA_LEVEL = (
    "owned_formula_authority_for_ID_1_ID_2_ID_4_ID_6_and_gated_ID_7_frontdoor_contextual_chain_parallel_frontdoor_set_and_downstream_confounded_chain_carried_Q_no_full_id_claim"
)
...
def _downstream_confounded_chain_formula(admg: ADMG, x: Sequence[str], y: Sequence[str]) -> Optional[CanonicalFormulaDiagnostic]:
    """Strict Step-79 ID-7 formula for a recursive non-frontdoor chain.

    Supported shape is deliberately narrow and matches the Step-76 oracle gap:

      X -> A -> B -> Y with X <-> B

    and no other observed nodes, directed edges, or bidirected edges.  The
    recursive engine already identified this branch, but it was counted as
    delegated formula authority.  Step 79 gives this exact carried-Q ID-7
    family an owned canonical formula without claiming arbitrary ID-7.

    Formula:

      P_x(y) = sum_{a,b} P(a|x) * sum_{x'} P(x') P(b|x',a) * P(y|x,a,b)
    """
    if len(x) != 1 or len(y) != 1:
        return None
    tx, out = x[0], y[0]
    ...
    paths = [p for p in _simple_directed_paths(admg, tx, out, max_len=4) if len(p) == 4]
    if len(paths) != 1:
        return None
    path = paths[0]
    a, b = path[1], path[2]
    required_directed = {(tx, a), (a, b), (b, out)}
    required_bidirected = {_bidirected_key(tx, b)}
    if set(admg.node_set) != set(path):
        return None
    if directed != required_directed or bidirected != required_bidirected:
        return None
    ...
    pre_factor = f"P({a} | {tx})"
    xp_factor = f"P({xp})"
    carried_factor = f"P({b} | {xp},{a})"
    outcome_factor = f"P({out} | {tx},{a},{b})"
    inner = f"sum_{{{xp}}} " + _product([xp_factor, carried_factor])
    rhs = f"sum_{{{','.join(outer_bound)}}} " + _product([pre_factor, inner, outcome_factor])

    inner_ast = Sum(
        [xp],
        Product([
            P([xp], label=xp_factor),
            P([b], given=[xp, a], label=carried_factor),
        ], label="canonical_downstream_confounded_chain_inner_carried_q_product_step79"),
        label="canonical_downstream_confounded_chain_sum_over_treatment_prime_step79",
    )
    ast = Sum(
        outer_bound,
        Product([
            P([a], given=[tx], label=pre_factor),
            inner_ast,
            P([out], given=[tx, a, b], label=outcome_factor),
        ], label="canonical_downstream_confounded_chain_product_step79"),
        label="canonical_downstream_confounded_chain_sum_over_mediators_step79",
    )
    terms = [pre_factor, xp_factor, carried_factor, outcome_factor]
    reason = "CANONICAL_ID7_DOWNSTREAM_CONFOUNDED_CHAIN_CARRIED_Q_FORMULA_STEP79"
    expr = _payload(
        "canonical_id7_downstream_confounded_chain_carried_q_formula",
        y,
        x,
        rhs,
        ast,
    ...
    trace = [
        {"rule": "ID-4", "status": "recognized_downstream_confounded_chain_district_decomposition", "pre_mediator": a, "confounded_mediator": b, "target": out},
        {"rule": "ID-7", "status": "identified_downstream_confounded_chain_carried_q_formula", "pre_mediator": a, "confounded_mediator": b, "target": out, "alpha_renamed_treatment": xp},
    ]
    return _diag(
        "identified_canonical_id7_downstream_confounded_chain_carried_q_formula_step79",
        True,
        formula=rhs,
        ast=ast,
        expression_json=expr,
        trace=trace,
        rule="ID-7",
        terminal="identified_downstream_confounded_chain_carried_q_formula",
        reason=reason,
        id7=1,
    )
...
def _frontdoor_formula(admg: ADMG, x: Sequence[str], y: Sequence[str]) -> Optional[CanonicalFormulaDiagnostic]:
    ...
    if parallel is not None:
        return parallel
    downstream_confounded = _downstream_confounded_chain_formula(admg, x, y)
    if downstream_confounded is not None:
        return downstream_confounded
...
def _canonical_id_formula_diagnostic(
    ...
    *,
    ...
    return _diag("pending_canonical_formula_unhandled_step67", False, trace=[{"rule": "ID-7", "status": "pending_general_id7"}], rule="ID-7", terminal="pending_general_id7", blocker_class="canonical_id7_pending", pending="complete_arbitrary_ID7_carried_Q_recursion_beyond_contextual_chain_parallel_frontdoor_sets_and_step79_downstream_confounded_chains", reason="CANONICAL_ID7_GENERAL_PENDING_STEP67")
