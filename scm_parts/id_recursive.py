...
"""Recursive ID routing diagnostics for CausalGate SCM.
...
def recursive_id_diagnostic(
    ...
    *,
    ...
    arbitrary subdistrict/q-factor recursion until CausalGate can carry symbolic
    Q-inputs safely.
    """
    x = _s(treatment)
    y = _s(outcome)
...
def _recursive_id_step_number(expr: Optional[RecursiveIDExpressionDiagnostic]) -> int:
    """Map an expression branch to CausalGate's incremental full-ID step label."""
