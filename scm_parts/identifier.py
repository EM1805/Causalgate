...
def write_identification_assets(out_dir: str = "out", scm_graph_path: Optional[str] = None, bridge = None, bridge_csv_path: Optional[str] = None, insights = None) -> Dict[str, str]:
    ...
    if bridge is None and bridge_csv_path and os.path.exists(bridge_csv_path):
        try:
            ...
            warnings.warn(f"[causalgate][warning] identification assets could not read bridge CSV {bridge_csv_path}: {type(exc).__name__}: {exc}", RuntimeWarning)
            bridge = None
    ...
    try:
        ...
        warnings.warn(f"[causalgate][warning] causal contract sync after scm-identify failed: {type(exc).__name__}: {exc}", RuntimeWarning)
    return paths
