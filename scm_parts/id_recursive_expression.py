...
class RecursiveIDExpressionDiagnostic:
    ...
    safe recursive branches that CausalGate can audit today:
...
def _lhs_variables_from_terms(q_input_terms: Sequence[str]) -> List[str]:
    """Return LHS variables carried by compact probability terms."""
    out: List[str] = []
    seen: Set[str] = set()
    for term in q_input_terms or []:
        lhs = _factor_lhs_variable(_s(term))
...
def _terms_excluding_intervened_lhs(q_input_terms: Sequence[str], x_set: Sequence[str]) -> List[str]:
    x = set(_dedupe(x_set))
    out: List[str] = []
    for term in q_input_terms or []:
        clean = _s(term)
        lhs = _factor_lhs_variable(clean)
        if clean and lhs not in x:
            out.append(clean)
    return out


def _display_bound_variable(node: str, *, external: bool = False) -> str:
    # External carried-Q variables are variables that were summed out by an
    # ancestor-reduction step but still appear in the carried source Q terms.
    # Prime them in display formulas so they cannot be confused with fixed
    # outer do-values.
    return _primed_name(node) if external else node


def _rename_external_bound_terms(terms: Sequence[str], external_bound: Sequence[str]) -> Tuple[List[str], List[str]]:
    aliases = {node: _display_bound_variable(node, external=True) for node in _dedupe(external_bound)}
    displayed: List[str] = []
    for term in terms or []:
        text = _s(term)
        for old, new_name in sorted(aliases.items(), key=lambda kv: len(kv[0]), reverse=True):
            text = _replace_symbol_token(text, old, new_name)
        displayed.append(text)
    return displayed, [aliases[n] for n in _dedupe(external_bound) if n in aliases]


def _q_input_truncated_formula_for_observed_dag(
    admg: ADMG,
    x_set: Sequence[str],
    y_set: Sequence[str],
    q_input_terms: Sequence[str],
) -> Tuple[str, List[str], List[str], FormulaAST]:
    """Truncated factorization over an observed DAG with a carried Q input.

    Earlier conservative branches used the ordinary observational DAG shortcut
    even inside a carried-Q recursive subproblem.  That is only correct when the
    input distribution is the original P(V).  For ID-7 subproblems the input may
    be Q[S'] or a marginal of Q[S']; source variables that have been removed by
    ancestor reduction must remain available as summed latent/source copies, and
    factors whose LHS is intervened on must be dropped.
    """
    order = topological_order(admg) or sorted(admg.node_set)
    x = set(_dedupe(x_set))
    y = set(_dedupe(y_set))
    retained_raw = _terms_excluding_intervened_lhs(q_input_terms, x)
    lhs = _lhs_variables_from_terms(q_input_terms)
    external_bound = [n for n in lhs if n not in admg.node_set and n not in x and n not in y]
    internal_bound = [n for n in order if n not in x and n not in y]
    displayed_terms, displayed_external = _rename_external_bound_terms(retained_raw, external_bound)
    sum_over = displayed_external + internal_bound
    product = " * ".join(displayed_terms) if displayed_terms else "1"
    formula = f"sum_{{{','.join(sum_over)}}} {product}" if sum_over else product
    formula_ast = _sum_if_needed_ast(sum_over, _product_terms_ast(displayed_terms, label="q_input_observed_dag_product"))
    return formula, sum_over, displayed_terms, formula_ast


def _q_factor_formula_for_district_from_carried_terms(
    admg: ADMG,
    ...
    """Return a Q-factor formula while preserving marginalized carried-Q source variables."""
    order = topological_order(admg) or sorted(admg.node_set)
    s = set(_dedupe(district))
    y = set(_dedupe(y_set))
    x = set(_dedupe(x_set))
    lhs = _lhs_variables_from_terms(q_input_terms)
    external_bound = [n for n in lhs if n not in admg.node_set and n not in x and n not in y]
    wanted_lhs = s | set(external_bound)
    retained_raw = []
    for term in q_input_terms or []:
        clean = _s(term)
        lhs_var = _factor_lhs_variable(clean)
        if clean and lhs_var in wanted_lhs and lhs_var not in x:
            retained_raw.append(clean)
    displayed_terms, displayed_external = _rename_external_bound_terms(retained_raw, external_bound)
    sum_over = displayed_external + [node for node in order if node in s and node not in y]
    product = " * ".join(displayed_terms) if displayed_terms else "1"
    formula = f"sum_{{{','.join(sum_over)}}} {product}" if sum_over else product
    return formula, sum_over, displayed_terms
...
def _q_factor_formula_for_district_from_terms(
    ...
    y = set(_dedupe(y_set))
    if q_input_terms:
        return _q_factor_formula_for_district_from_carried_terms(admg, district, y_set, q_input_terms, x_set=x_set)
    terms: List[str] = []
    previous: List[str] = []
    for node in order:
        if node in s:
...
def _marginalized_q_input_ast_for_scope(
    admg: ADMG,
    scope: Sequence[str],
    *,
    source_scope: Sequence[str],
    ...
    """Build Q[scope] as a marginal of a larger carried Q input.

    This is the operational Step-75 piece needed by arbitrary ID-7 carried-Q
    recursion: ancestor reduction inside Q[S'] should pass sum_{S' minus A} Q[S']
    forward, not merely project away source terms whose left-hand variable was
    removed.
    """
    clean_scope = [n for n in _dedupe(scope) if n in admg.node_set]
    if not clean_scope:
        return None
    clean_source_scope = _dedupe(source_scope)
    removed = [n for n in clean_source_scope if n not in set(clean_scope)]
    product = _product_terms_ast(source_terms, label="marginalized_carried_q_source_product")
    inner = _sum_if_needed_ast(removed, product)
    q_ast = Q(
        clean_scope,
        terms=(inner,),
        q_input=source_name,
        label="marginalized_carried_q_input_step75",
    )
    meta = dict(q_ast.metadata)
    meta.update({
        "operational_carried_q_ast_step53": 1,
        "arbitrary_id7_carried_q_recursion_step75": 1,
        "q_input": source_name,
        "q_input_scope": clean_scope,
        "source_scope": clean_source_scope,
        "marginalized_source_variables": removed,
    })
    return FormulaAST(
        q_ast.node_type,
        variables=q_ast.variables,
        conditioned_on=q_ast.conditioned_on,
        interventions=q_ast.interventions,
        bound_variables=q_ast.bound_variables,
        children=q_ast.children,
        label=q_ast.label,
        metadata=meta,
    ).to_dict()
...
def _recursive_id_expression(
    ...
    *,
    ...
    if removed_non_ancestors:
        ...
        # Step 75: preserve the full carried source product through ancestor
        # reduction.  Variables removed from the graph remain available as
        # summed source copies inside Q[An(Y)] instead of being silently dropped.
        reduced_q_terms = list(q_terms) if q_terms else []
        reduced_q_ast = _marginalized_q_input_ast_for_scope(reduced, reduced_q_scope, source_scope=q_scope, source_terms=q_terms, source_name=q_name) if reduced_q_terms and reduced_q_scope else None
        trace.append(_proof_step("q_input_ancestor_marginalization_step75", "applied", source_q_input=q_name, source_q_scope="|".join(q_scope), reduced_q_scope="|".join(reduced_q_scope), marginalized_source_variables="|".join([n for n in q_scope if n not in set(reduced_q_scope)]))) if q_terms else None
    ...
    if not admg.bidirected_edges:
        if q_terms:
            rhs, eliminated, retained_terms, inner_ast = _q_input_truncated_formula_for_observed_dag(admg, x, y, q_terms)
            if rhs:
                trace.append(_proof_step("observed_dag_truncated_factorization", "identified", eliminated_nodes="|".join(eliminated), q_input=q_name, formula_ast_source="carried_q_truncated_factorization_step75"))
                formula_ast = _wrap_do_ast(x, inner_ast)
                reason = "Q_INPUT_OBSERVED_DAG_TRUNCATED_FACTORIZATION_STEP75"
                return RecursiveIDExpressionDiagnostic("identified_q_input_observed_dag_truncated_factorization_step75", True, rhs, expression_json=_expression_payload(kind="q_input_observed_dag_truncated_factorization_step75", y_set=y, x_set=x, formula=rhs, sum_over=eliminated, product_terms=retained_terms, reason_codes=reason, formula_ast=formula_ast), trace_json=_json_formula({"trace": trace}), depth=depth, pending_operator="none", reason_codes=reason)
    ...
    if len(remaining_districts) == 1:
        ...
        if any(set(s) == set(d) for d in full_districts):
            q_formula, sum_over, terms = _q_factor_formula_for_district_from_terms(admg, s, y, q_terms, x_set=x)
            if q_formula:
