...
"""Step 80 Full-ID readiness matrix for CausalGate SCM-ID.
...
def full_id_readiness_cases() -> List[IDFullReadinessCase]:
    ...
    captures current safe behavior: identified where CausalGate has authority,
    ...
    return [
        ...
        IDFullReadinessCase(
            case_id="id_downstream_confounded_chain_canonical_id7_step79",
            description="Strict non-frontdoor chain X -> A -> B -> Y with X <-> B uses Step-79 owned canonical carried-Q ID-7 authority.",
            query_kind="id",
            ...
            expected_primary_formula_authority="id_canonical_formula_step60",
            expected_canonical_rules=("ID-4", "ID-7"),
            expected_formula_contains=("P_{do(X)}(Y)", "sum_{A,B}", "sum_{X_prime}", "P(B | X_prime,A)", "P(Y | X,A,B)"),
            expected_canonical_formula_used=1,
            expected_canonical_id7_used=1,
        ),
        IDFullReadinessCase(
            case_id="id_step80_audited_fuzz_closure_recursive_id7",
            description="Previously delegated four-node recursive ID-7 fuzz closure is promoted to finite audited authority, not arbitrary Full-ID authority.",
            query_kind="id",
            graph=_case_graph(["X", "A", "B", "Y"], [("X", "A"), ("A", "B"), ("B", "Y")], [("X", "B"), ("A", "Y")]),
            treatments=("X",),
            outcomes=("Y",),
            expected_identified=True,
            expected_status="identified_recursive_district_decomposition",
            ...
            expected_canonical_rules=("ID-4", "ID-7"),
            expected_formula_contains=("P_{do(X)}(Y)", "sum_{A,B}", "sum_{X'}", "P(Y | X,A,B)", "P(B | X',A)"),
            expected_canonical_formula_used=0,
        ),
        IDFullReadinessCase(
            case_id="id_arbitrary_carried_q_marginalization_step75",
            description="Non-frontdoor ADMG requiring ID-7 carried-Q recursion plus ancestor marginalization of a source variable inside Q[S'].",
            query_kind="id",
            graph=_case_graph(["A", "B", "C", "D", "E"], [("A", "B"), ("A", "C"), ("C", "D"), ("D", "E")], [("A", "D"), ("A", "E"), ("B", "D")]),
            treatments=("A",),
            outcomes=("E",),
            expected_identified=True,
            expected_primary_formula_authority="recursive_id_set_expression_diagnostic_step75",
            expected_canonical_rules=("ID-2", "ID-4", "ID-7", "ID-6"),
            expected_formula_contains=("P_{do(A)}(E)", "sum_{C,D}", "sum_{A'} P(A') * P(D | A',C)", "sum_{A'} P(A') * P(E | A',C,D)"),
            expected_canonical_formula_used=0,
        ),
        IDFullReadinessCase(
...
def evaluate_full_id_readiness_case(case: IDFullReadinessCase) -> IDFullReadinessRow:
    ...
    trace_authority = recursive_trace_authority_for_result(result, query_kind=case.query_kind)
    ...
    expected_claim_allowed = int(flags.get("full_recursive_id_implemented") == 1 and flags.get("full_id_claim_allowed") == 1)
    if full_id_claim_allowed != expected_claim_allowed:
        failures.append(f"full_id_claim_allowed={full_id_claim_allowed} expected={expected_claim_allowed}")
    ...
    return IDFullReadinessRow(
        ...
        expected_full_id_claim_allowed=expected_claim_allowed,
        formula=formula,
...
def run_full_id_readiness_matrix(cases: Iterable[IDFullReadinessCase] | None = None) -> Dict[str, object]:
    ...
    return {
        ...
        "full_id_claim_reason": (
            "Step 94 promotes the global Full-ID claim only after this readiness surface, oracle/fuzz, failure coverage, recursive authority, kernel, and controlled flip gates are green."
        ),
        "remaining_required_before_full_id_claim": [
            "oracle/parity coverage for arbitrary ID-7 carried-Q recursion beyond the Step-75 non-frontdoor carried-Q marginalization case, Step-79 strict downstream-confounded chain, and Step-80 audited finite fuzz closures",
