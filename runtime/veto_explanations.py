...
def build_veto_explanations(intent: Dict[str, Any], context_flags: Dict[str, Any], paths: List[Dict[str, Any]], decision: Dict[str, Any], mitigations: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for p in paths[:3]:
        line = {
            ...
            'causal_authority_summary': {
                'veto_authority_class': p.get('veto_authority_class', 'policy_veto'),
                'causal_veto_claim_allowed': bool(p.get('causal_veto_claim_allowed', False)),
                'reason': p.get('causal_veto_claim_reason', ''),
                'pearl_veto_authority_class': p.get('pearl_veto_authority_class', 'not_causal_veto'),
                'pearl_complete_veto_claim_allowed': bool(p.get('pearl_complete_veto_claim_allowed', False)),
                'pearl_complete_reason': p.get('pearl_complete_veto_claim_reason', ''),
            },
            'caveats': [
                c for c in [
    ...
    operator = (
        ...
        f"veto_authority_class={top.get('veto_authority_class', 'policy_veto')}; "
        f"causal_veto_claim_allowed={bool(top.get('causal_veto_claim_allowed', False))}; "
        f"pearl_veto_authority_class={top.get('pearl_veto_authority_class', 'not_causal_veto')}; "
        f"pearl_complete_veto_claim_allowed={bool(top.get('pearl_complete_veto_claim_allowed', False))}"
    )
    return {
        'human_summary': human,
        'operator_summary': operator,
