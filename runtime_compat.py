"""Runtime compatibility checks for CausalGate.
...
class RuntimeCompatibilityError(RuntimeError):
    """Raised when CausalGate is running on an unsupported runtime."""
...
def assert_supported_python() -> None:
    ...
    CausalGate is validated on Python 3.10-3.12 and is allowed on Python 3.13 when
    ...
    if sys.version_info < _MIN_PYTHON or sys.version_info >= _MAX_PYTHON_EXCLUSIVE:
        ...
        raise RuntimeCompatibilityError(
            "Unsupported Python runtime for CausalGate: "
...
def assert_scientific_stack() -> None:
    ...
    if missing:
        raise RuntimeCompatibilityError(
            "Missing scientific dependency for CausalGate: "
            + ", ".join(missing)
    ...
    if sys.version_info >= (3, 13):
        if not ((2, 1, 3) <= numpy_tuple < (2, 4)) or not ((2, 2, 3) <= pandas_tuple < (2, 4)):
            raise RuntimeCompatibilityError(
                "Python 3.13 requires compatible pandas/numpy wheels for CausalGate. "
        ...
        if not ((1, 26) <= numpy_tuple < (2, 4)) or not ((2, 2, 3) <= pandas_tuple < (2, 4)):
            raise RuntimeCompatibilityError(
                "Incompatible pandas/numpy versions for CausalGate. "
                f"Detected numpy=={numpy_version}, pandas=={pandas_version}. "
