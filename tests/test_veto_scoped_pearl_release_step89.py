...
from common.pearl_release_manifest import code_manifest_digest, scoped_pearl_release_digest
...
def test_step89_runtime_allows_scoped_pearl_only_with_verified_release_manifest(tmp_path):
    ...
    assert authority["scoped_pearl_manifest_verified"] == 1
    assert authority["scoped_pearl_release_verified"] == 1
...
def test_step89_runtime_blocks_scoped_pearl_without_release_manifest(tmp_path):
    cards = tmp_path / "cards.jsonl"
    _write_cards(cards, [_card(_bundle(release={}))])
    ...
    assert "scoped_pearl_release_verified" in readiness["pearl_scoped_veto_claim_reason"]
    with pytest.raises(ValueError, match="scoped_pearl_release_verified"):
        assert_pearl_scoped_veto_claim_allowed(paths, {"decision": "HARD_BLOCK"})


def test_step89_runtime_blocks_rehashed_release_manifest_with_code_drift(tmp_path):
    cards = tmp_path / "cards.jsonl"
    manifest = build_current_scoped_pearl_manifest()
    release = build_current_scoped_pearl_release_manifest(scoped_manifest=manifest)
    tampered = dict(release)
    code_digests = dict(tampered["code_file_digests"])
    code_digests["runtime/causal_authority.py"] = "f" * 64
    ...
    _write_cards(cards, [_card(_bundle(manifest=manifest, release=tampered))])
    ...
    authority = paths[0]["pearl_scoped_veto_authority"]
    assert readiness["pearl_scoped_veto_claim_allowed"] is False
    assert authority["scoped_pearl_release_verified"] == 0
    assert authority["scoped_pearl_release_digest_valid"] == 1
    assert "code_file_digests" in authority["scoped_pearl_release_mismatched_fields"]
    assert "scoped_pearl_release_verified" in readiness["pearl_scoped_veto_claim_reason"]
