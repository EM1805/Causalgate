    ...
    for path in [data_path, os.path.join(out_dir, "data_clean.csv"), "data.csv", os.path.join(out_dir, "demo_data.csv")]:
        if path and os.path.exists(path):
            try:
                ...
                warnings.warn(f"[causalgate][warning] SCM fit could not read data file {path}: {type(exc).__name__}: {exc}", RuntimeWarning)
                continue
    return pd.DataFrame()
...
def fit_scm_models(out_dir: str = "out", scm_graph_path: Optional[str] = None, data_path: Optional[str] = None) -> Dict[str, str]:
    ...
    if len(skipped_df):
        warnings.warn(f"[causalgate][warning] SCM fit skipped {len(skipped_df)} node(s); see {paths['fit_manifest_json']}", RuntimeWarning)
    return paths
