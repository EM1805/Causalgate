...
def load_data(data_path: Optional[str], out_dir: str) -> pd.DataFrame:
    ...
    for path in candidates:
        ...
        try:
            ...
            warnings.warn(f"[causalgate][warning] SCM builder could not read data file {msg}", RuntimeWarning)
            continue
    warnings.warn(
        "[causalgate][warning] SCM builder found no readable data file. "
...
def load_dag(dag_path: Optional[str], out_dir: str):
    ...
    try:
        ...
        warnings.warn(f"[causalgate][warning] SCM builder could not load offline prior graph {path}: {type(exc).__name__}: {exc}", RuntimeWarning)
        return None
