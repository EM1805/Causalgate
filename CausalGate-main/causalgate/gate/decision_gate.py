...
from causalgate.contracts import ActionPackage, DecisionPackage, decision_package_from_runtime, normalize_action_package
...
def _resolve_project_path(path_value: str) -> str:
    ...
    Online callers often import CausalGate from another working directory. The
...
class DecisionGate:
    """Agentic facade over the existing CausalGate runtime veto gateway.
    ...
    def _run_counterfactual(self, action: ActionPackage) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if engine is None:
                from causalgate.causal_core.counterfactual import CounterfactualEngine
        ...
        if recommended and current and recommended != current and status == "alternative_recommended":
            ...
            if decision.decision == "allow":
                ...
                decision.llm_instruction = (
                    ...
                    "State that CausalGate selected the safer/higher-utility alternative."
                )
    ...
    def _enrich_with_identification(
        ...
        if decision.decision == "allow":
            if risk in _HIGH_RISK or reversibility in _IRREVERSIBLE:
                ...
                decision.llm_instruction = (
                    "Do not execute the action yet. Explain that CausalGate lacks identifiable causal support "
    ...
    def _apply_risk_policy(self, decision: DecisionPackage, action: ActionPackage) -> DecisionPackage:
        ...
        try:
            ...
            if policy is None:
                from causalgate.risk_policy import CausalEvidenceByRiskPolicy
            ...
            if new_decision != old_decision:
                ...
                if new_decision == "veto":
                    decision.llm_instruction = (
                        "Do not execute this action. CausalGate's risk policy requires stronger trusted evidence "
                        "before any tool call can run. Offer the safer recommended action or route to approval."
                    )
                    decision.confidence = "high"
                elif new_decision == "abstain":
                    decision.llm_instruction = (
                        "Do not execute yet. CausalGate needs more causal support, trusted approval, or mitigation evidence "
                        "for this higher-risk action."
                    )
                    decision.confidence = "low"
                elif new_decision == "warn":
                    decision.llm_instruction = (
                        "Proceed only with warning, mitigation, and audit because CausalGate's risk policy found missing evidence."
                    )
                    if decision.confidence == "high":
                        decision.confidence = "medium"
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate an ActionPackage through CausalGate's agentic Decision Gate.")
