...
"""Stable online estimation adapter for CausalGate runtime boundaries.

This facade connects the online/runtime layer to the existing ``estimation_parts``
backend without weakening CausalGate's conservative epistemics.
...
class EstimationQuery:
    ...
    query_id: str = ""
    source: str = "causalgate.causal_core.estimation"
    ...
    def from_payload(cls, payload: Mapping[str, Any] | None) -> "EstimationQuery":
        ...
        return cls(
            ...
            source=_s(payload.get("source")) or "causalgate.causal_core.estimation",
            raw_payload=payload,
        )
...
def _import_pandas_safely():
    ...
    temporarily removed, then restore ``sys.path`` before importing CausalGate's
    own backend modules.
    """
