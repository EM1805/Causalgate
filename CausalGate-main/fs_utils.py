...
def _touch_probe(directory):
    try:
        ...
        probe = directory / ".causalgate_write_probe"
...
def ensure_writable_dir(path, fallback_prefix="causalgate_out_"):
    ...
    print("[causalgate:fs] Output directory is not writable: %s. Using fallback: %s" % (requested, fallback_root))
...
def ensure_writable_parent(file_path, fallback_dir=None):
    ...
    base_dir = Path(fallback_dir) if fallback_dir is not None else ensure_writable_dir(Path(tempfile.gettempdir()) / "causalgate_outputs")
    base_dir.mkdir(parents=True, exist_ok=True)
    fallback = base_dir / requested.name
    print("[causalgate:fs] Output path is not writable: %s. Using fallback: %s" % (requested, fallback))
    return fallback
